...
def consultar_tiempo_elaboracion():
    ...
    if(request.method == 'POST'):
        xtiempo = float(request.form['tiempo'])
        band = True
        xrecetas = Receta.query.order_by(-Receta.tiempo).limit(xtiempo)
        
        return render_template('consultar_tiempo_elaboracion.html', bandera = band, recetas = xrecetas, usuario = usuario_actual)
    else:
        return render_template('consultar_tiempo_elaboracion.html', bandera = band, recetas = xrecetas, usuario = usuario_actual)
